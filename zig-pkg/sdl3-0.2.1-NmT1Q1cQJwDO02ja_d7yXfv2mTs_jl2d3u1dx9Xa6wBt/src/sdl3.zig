...
/// If there are errors using this, you need to ensure `ext_net` is set to `true`.
pub const image = @import("image.zig");

/// Initialization data that can be used for callbacks.
///
/// ## Remarks
/// Arguments and environment are not provided as the init function contains the arguments, and SDL provides a mechanism for interacting with environment variables.
///
/// ## Version
...
pub const main_callbacks = @import("main_callbacks.zig");
...
pub const shadercross = @import("shadercross.zig");
...
/// TODO documentation.
/// If there are errors using this, you need to ensure `ext_ttf` is set to `true`.
...
///
/// ## Function Parameters
/// * `app_state`: User data, initialized by the app in `SDL_AppInit()`.
...
///
/// ## Function Parameters
/// * `init`: Initialization struct similar to zig's juicy main.
///
/// ## Return Value
/// Returns the app state along with `AppResult.failure` to terminate with an error, `AppResult.success` to terminate with success, `AppResult.run` to continue.
///
/// ## Remarks
/// These are used by `main.enterAppMainCallbacks()`.
/// This mechanism operates behind the scenes for apps using the optional main callbacks.
...
pub fn AppInitCallback(
    ...
    return *const fn (
        init: Init,
    ) anyerror!struct { UserData, AppResult };
}
...
///
/// ## Function Parameters
/// * `app_state`: User data, initialized by the app in `SDL_AppInit()`. This will be `null` if the init function never finished successfully.
...
pub fn runOnMainThread(
    ...
    const ret = c.SDL_RunOnMainThread(Cb.run, @ptrCast(user_data), wait_complete);
    return errors.wrapCallBool(ret);
}
...
fn allocRealloc(mem: ?[*]u8, size: usize) ?[*]u8 {
    ...
    @memcpy(new_mem[0..@min(size, old_allocation.size)], old_mem[0..@min(size, old_allocation.size)]);
...
test {
    errors.error_callback = null;
    comptime {
        _ = @import("assert.zig");
        _ = @import("async_io.zig");
        _ = @import("atomic.zig");
        _ = @import("audio.zig");
        _ = @import("bits.zig");
        _ = @import("blend_mode.zig");
        _ = @import("camera.zig");
        _ = @import("clipboard.zig");
        _ = @import("cpu_info.zig");
        _ = @import("dialog.zig");
        _ = @import("endian.zig");
        _ = @import("errors.zig");
        _ = @import("events.zig");
        ...
        _ = @import("guid.zig");
        _ = @import("haptic.zig");
        _ = @import("hid_api.zig");
        _ = @import("hints.zig");
        _ = @import("image.zig");
        _ = @import("intrin.zig");
        _ = @import("io_stream.zig");
        _ = @import("joystick.zig");
        ...
        _ = @import("log.zig");
        _ = @import("main.zig");
        // _ = @import("main_callbacks.zig");
        _ = @import("message_box.zig");
        _ = @import("metal.zig");
        _ = @import("misc.zig");
        _ = @import("mouse.zig");
        _ = @import("mutex.zig");
        ...
        _ = @import("rect.zig");
        _ = @import("render.zig");
        _ = @import("scancode.zig");
        _ = @import("sensor.zig");
        _ = @import("shadercross.zig");
