...
pub fn build(b: *std.Build) void {
    ...
    // NOTE: Android system libraries (android, log, dl) are deliberately NOT linked here. In a
    // STATIC build zig records them as archive members, and the final link then warns that each
    // NDK .so "is neither ET_REL nor LLVM bitcode". They belong to whoever produces the shared
    // object — the engine links them on its own android arm.
    if (ios) {
