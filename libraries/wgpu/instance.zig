...
// Layout must match WGPUInstanceExtras in the pinned wgpu-native v29.0.1.1 wgpu.h exactly —
// there is no compiler check across this boundary, and a drifted tail (as this binding once had:
// a phantom dxil_path field and a missing dx12PresentationSystem/budget/displayHandle tail) makes
// wgpu read stack garbage past the fields Zig wrote.
pub const InstanceExtras = extern struct {
    chain: ChainedStruct = ChainedStruct{
        .s_type = SType.instance_extras,
    },
    backends: InstanceBackend = InstanceBackends.all,
    flags: InstanceFlag = InstanceFlags.default,
    dx12_shader_compiler: Dx12Compiler = .undefined,
    gles3_minor_version: Gles3MinorVersion = .automatic,
    gl_fence_behavior: GLFenceBehaviour = .gl_fence_behaviour_normal,
    dxc_path: StringView = StringView{},
    dxc_max_shader_model: DxcMaxShaderModel = .dxc_max_shader_model_v6_0,
...
// Matches WGPUInstanceDescriptor in the pinned v29.0.1.1 webgpu.h (the former
// `features: InstanceCapabilities` field was a stale layout from an older header revision —
// same total size, so zero-initialized values happened to be read as count 0 + null pointers).
...
pub const Instance = opaque {
    ...
    pub fn requestAdapterSync(self: *Instance, options: ?*const RequestAdapterOptions, polling_interval_nanoseconds: u64) RequestAdapterResponse {
        ...
        self.processEvents();
        while (!completed) {
            _misc.sleepNanos(polling_interval_nanoseconds); // cross-platform (posix nanosleep/timespec are unavailable on Windows)
            self.processEvents();
        }
