...
/// Cross-platform sleep for the synchronous adapter/device request polling loops. POSIX uses libc
/// nanosleep; Windows has no nanosleep (and `std.c.timespec` is `void` there), so it uses the WinAPI
/// `Sleep` (millisecond granularity, rounded up). Zig 0.16 removed the old `std.time.sleep` helper.
pub fn sleepNanos(nanoseconds: u64) void {
    if (builtin.os.tag == .windows) {
        var ms: u32 = @intCast(@min(nanoseconds / 1_000_000, @as(u64, std.math.maxInt(u32))));
        if (ms == 0 and nanoseconds != 0) ms = 1;
        Sleep(ms);
    } else {
...
pub const FeatureName = enum(u32) {
    ...
    // wgpu-native extras
    immediates = 0x00030001, // renamed from push_constants in wgpu-native 29.0.1 (same value)
