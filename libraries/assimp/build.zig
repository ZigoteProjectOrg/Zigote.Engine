...
pub fn build(b: *std.Build) !void {
    ...
    // Zig 0.16: Compile.linkLibC/linkLibCpp were removed — set on the root module instead.
    ...
    // External zlib (modern, from source) instead of assimp's bundled contrib/zlib. ASSIMP_BUILD_NO_OWN_ZLIB
    // makes assimp #include the linked zlib's <zlib.h>; the contrib/zlib include path is intentionally
    // omitted above so the ancient bundled headers don't shadow it.
    const zlib_dep = b.dependency("zlib", .{ .target = target, .optimize = optimize });
    lib.root_module.linkLibrary(zlib_dep.artifact("z"));
    lib.root_module.addCMacro("ASSIMP_BUILD_NO_OWN_ZLIB", "");
    ...
    inline for (comptime std.meta.declarations(sources.libraries)) |ext_lib| {
        // Bundled contrib/zlib is replaced by the external modern zlib (linked above) — skip it.
        if (comptime std.mem.eql(u8, ext_lib.name, "zlib")) continue;
        // contrib unzip/zip are old K&R-style C: pin gnu17 so Zig 0.16's clang (which defaults to C23,
        // where K&R function definitions were removed) accepts them. C++ contribs keep defaults.
        const is_c = comptime (std.mem.eql(u8, ext_lib.name, "unzip") or
            std.mem.eql(u8, ext_lib.name, "zip"));
        lib.root_module.addCSourceFiles(.{
            .root = assimp.path(""),
            .files = &@field(sources.libraries, ext_lib.name),
            .flags = if (is_c) &.{"-std=gnu17"} else &.{},
        });
    }
    ...
    // Upstream's static-example-c/-cpp executables removed: this fork only ships the `assimp`
    // library artifact, and their linkLibC()/linkLibCpp() calls don't compile on Zig 0.16.
}
