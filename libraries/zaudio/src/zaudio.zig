...
// Zigote vendor fixup (Zig 0.16): `std.Thread.Mutex` was removed in favour of the io-parameterised
// `std.Io.Mutex`. These allocator callbacks run off the real-time audio path (only at device init/
// uninit), so a tiny spinlock guarding the allocation map is sufficient and keeps the call sites
// (`mem_mutex.lock()` / `.unlock()`) unchanged.
