/// flecs ECS implementation module for the Zigote FFI layer.
///
/// Contains the actual logic for all zigote_ecs_* operations. The `export fn` wrappers
/// that form the C ABI surface live in root.zig (matching the audio.zig/physics.zig pattern).
///
...
pub fn ownsId(world: u64, entity: u64, component: u64) u8 {
    return if (flecs.ecs_owns_id(worldFromHandle(world), entity, component)) 1 else 0;
}

pub fn setId(world: u64, entity: u64, component: u64, data: [*c]const u8, size: usize) void {
    _ = flecs.ecs_set_id(worldFromHandle(world), entity, component, size, data);
}

pub fn getId(world: u64, entity: u64, component: u64) [*c]const u8 {
    return @ptrCast(flecs.ecs_get_id(worldFromHandle(world), entity, component));
}

pub fn getMutId(world: u64, entity: u64, component: u64) [*c]u8 {
    return @ptrCast(flecs.ecs_get_mut_id(worldFromHandle(world), entity, component));
}

pub fn ensureId(world: u64, entity: u64, component: u64, size: usize) [*c]u8 {
    return @ptrCast(flecs.ecs_ensure_id(worldFromHandle(world), entity, component, size));
}

pub fn modifiedId(world: u64, entity: u64, component: u64) void {
    flecs.ecs_modified_id(worldFromHandle(world), entity, component);
}

// ── Queries — pull model ───────────────────────────────────────────────────────

pub fn queryCreate(world: u64, components: [*c]const u64, count: u32) u64 {
...
pub fn queryDestroy(query: u64) void {
    flecs.ecs_query_fini(queryFromHandle(query));
}

pub fn queryIter(world: u64, query: u64) u64 {
...
pub fn iterFieldFromPtr(iter_ptr: usize, term_index: i32, size: usize) [*c]u8 {
    const it: *flecs.ecs_iter_t = @ptrFromInt(iter_ptr);
    return @ptrCast(flecs.ecs_field_w_size(it, size, @intCast(term_index)));
}

pub fn iterCountFromPtr(iter_ptr: usize) i32 {
    const it: *flecs.ecs_iter_t = @ptrFromInt(iter_ptr);
    return it.count;
}

pub fn iterEntitiesFromPtr(iter_ptr: usize) [*c]const u64 {
    const it: *flecs.ecs_iter_t = @ptrFromInt(iter_ptr);
    return @ptrCast(it.entities);
}

pub fn iterCtx(iter_ptr: usize) u64 {
    const it: *flecs.ecs_iter_t = @ptrFromInt(iter_ptr);
...
pub fn makePair(relation: u64, target: u64) u64 {
    return flecs.ecs_make_pair(relation, target);
}

pub fn addPair(world: u64, e: u64, relation: u64, target: u64) void {
    flecs.ecs_add_pair(worldFromHandle(world), e, relation, target);
}

pub fn removePair(world: u64, e: u64, relation: u64, target: u64) void {
    flecs.ecs_remove_pair(worldFromHandle(world), e, relation, target);
}

pub fn hasPair(world: u64, e: u64, relation: u64, target: u64) u8 {
    return if (flecs.ecs_has_pair(worldFromHandle(world), e, relation, target)) 1 else 0;
}

pub fn newWPair(world: u64, relation: u64, target: u64) u64 {
    return flecs.ecs_new_w_pair(worldFromHandle(world), relation, target);
}

pub fn setParent(world: u64, child: u64, parent: u64) void {
    flecs.ecs_add_pair(worldFromHandle(world), child, flecs.EcsChildOf, parent);
}

pub fn getParent(world: u64, child: u64) u64 {
    return flecs.ecs_get_target(worldFromHandle(world), child, flecs.EcsChildOf, 0);
}

pub fn isA(world: u64, e: u64, base: u64) void {
    flecs.ecs_add_pair(worldFromHandle(world), e, flecs.EcsIsA, base);
}

pub fn newPrefab(world: u64, name: [*c]const u8) u64 {
...
pub fn instantiate(world: u64, prefab: u64) u64 {
...
pub fn builtinPhase(which: u8) u64 {
    return switch (which) {
        0 => flecs.EcsOnStart,
        1 => flecs.EcsPreFrame,
        2 => flecs.EcsOnLoad,
